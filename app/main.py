...
st.caption(
    "Motor de reglas para altas, bajas, análisis paralelo y simulación de impacto económico"
)
...
def load_data():
    repo = Repo()
    troqueles_df = repo.table_df("troqueles")
    convenio_df = repo.table_df("convenio_oyte")
    liquidaciones_df = repo.table_df("liquidaciones")
    return troqueles_df, convenio_df, liquidaciones_df
...
tab_panel, tab_simular, tab_historial, tab_datos = st.tabs(
    ["Panel financiero", "Nueva simulación", "Historial", "Datos"]
)
...
with tab_panel:
    ...
    if not troqueles.empty and "monodroga" in troqueles.columns:
        count = (
            troqueles.groupby("monodroga", as_index=False)
            .size()
        ...
        fig = px.bar(
            ...
            x="monodroga",
            y="size",
            labels={"size": "Cantidad", "monodroga": "Monodroga"},
        )
...
with tab_simular:
    ...
    with col_form:
        if not troqueles.empty and "codigo_troquel" in troqueles.columns:
            ...
            codigos = []
            codigo = st.text_input("Código de troquel")
    ...
    with col_prev:
        ...
        if codigo and not troqueles.empty and "codigo_troquel" in troqueles.columns:
            ...
            if not row.empty:
                ...
                st.write(
                    f"**Potencia/Forma:** {r.get('potencia', '')} / {r.get('forma_farmacologica', '')}"
                )
            ...
            st.info("No hay datos cargados para validar.")
    ...
    if ejecutar and codigo:
        try:
            ...
            b.metric(
                ...
                f"${result.facturacion_proyectada_anual:,.0f}",
            )
            c.metric(
                "Impacto neto",
                f"${result.facturacion_proyectada_anual - result.facturacion_actual_anual:,.0f}",
            )
...
with tab_historial:
    ...
    try:
        ...
        if not hist.empty:
            if "fecha_corrida" in hist.columns:
                hist = hist.sort_values("fecha_corrida", ascending=False)
